...
laws_dict = {'entrepreneurship_laws': ['SG.LAW.CRDD.GR', 'SG.CNT.SIGN.EQ', 'SG.BUS.REGT.EQ',
                                       'SG.OPN.BANK.EQ'],
             'workplace_laws': ['SG.GET.JOBS.EQ', "SG.LAW.NODC.HR", "SG.LEG.SXHR.EM",
                                "SG.PEN.SXHR.EM"],
             'pay_laws': ['SG.LAW.EQRM.WK', 'SG.NGT.WORK.EQ', 'SG.DNG.WORK.DN.EQ',
                          'SG.IND.WORK.EQ']}
...
def make_marimekko(indicators: list) -> pd.DataFrame:
    ...
    df = (LAWS
          .loc[lambda d: d.indicator_code.isin(indicators)]
          .dropna(subset='value')
          ...
          .loc[:, ['indicator_name', 'entity_name', 'year', 'value_label', 'value', 'female_pop']]
          )
    ...
    df = (df.sort_values(by = 'female_pop', ascending=False)
          .assign(female_pop_annot=lambda d: (d.female_pop / 1000000))
          )
...
def chart_laws_marimekko() -> None:
    ...
    for law_type, indicators in laws_dict.items():
        (make_marimekko(indicators)
         .to_csv(f'{PATHS.output}/laws_{law_type}.csv', index=False)
         )
