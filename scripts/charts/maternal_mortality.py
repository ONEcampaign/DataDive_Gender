...
def chart_pictogram_world() -> None:
    ...
    (
        REGIONS.loc[
            lambda d: (d.parameter == "maternal_deaths_summation_of_country_estimates")
            & (d.region == "world")
            & (d.year == 2020),
            ["region", "year", "value"],
        ]
        .assign(
            value=lambda d: d.value.round(0),
...
def chart_pictogram_low_middle_income() -> None:
    ...
    (
        ...
        .pivot(index="year", columns="region", values="value")
        .assign(
            low=lambda d: d["Low income"] + d["Lower middle income"],
            diff=lambda d: d["world"] - d["low"],
        )
        .rename(
            columns={"diff": "Rest of the world", "low": "Low and Lower middle income"}
        )
        .reset_index()
...
def calculate_pct_change(
    ...
    year_val_dict = (
        df[lambda d: (d.year == year)].set_index(group_col)[val_col].to_dict()
    )
    return df.assign(
        val_year=lambda d: d[group_col].map(year_val_dict),
...
def chart_line_change_in_mmr() -> None:
    ...
    countries_df = (
        COUNTRIES.loc[
            lambda d: (d.parameter == "mmr") & (d.country.isin(country_list)),
    ...
    df_regions = (
        REGIONS.loc[
            lambda d: (d.parameter == "mmr") & (d.region.isin(region_list)),
    ...
    (
        pd.merge(countries_df, df_regions, on="year", how="outer").to_csv(
            f"{PATHS.output}/mmr_line_change_in_mmr.csv", index=False
        )
    )
